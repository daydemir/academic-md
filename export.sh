...
pandoc -s "$MD_FILE" -o "$TEX_FILE" --citeproc $CSL_ARG
...
pandoc -s "$MD_FILE" -o "$HTML_FILE" --mathjax --citeproc $CSL_ARG

# Convert Markdown directly to EPUB (using Pandoc, with MathML for math rendering)
pandoc -s "$MD_FILE" -o "$EPUB_FILE" --mathml --citeproc $CSL_ARG
